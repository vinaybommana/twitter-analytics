...
def main():
    ...
    with codecs.open('step_eight_output_rank.csv', 'w+', 'utf-8') as o:
        ...
        for row in sorted_rows:
            line = str(row[4]) + "," + str(row[1]) + "," + str(rank) + str("\n")
            o.write(line)
            rank += 1

    # step eight output with no of tweets and retweets
    with codecs.open('step_eight_output_info', 'w+', 'utf-8') as o:
        o.write("Global_rank\t" + "," + "screen_name\t" + "," + "No of Tweets\t" + "," + "No of retweets\t" + "," + "Global_influence_score\n")
        rank = 1
        for row in sorted_rows:
            line = str(rank) + "," + str(row[1]) + "," + str(row[2]) + "," + str(row[3]) + "," + str(row[4])
            line += str("\n")
            o.write(line)
            rank += 1
